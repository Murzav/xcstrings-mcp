...
fn explicit_leaf_submission_updates_only_its_destination_and_reports_identity() {
    ...
    let translations: Vec<xcstrings_mcp::model::translation::CompletedTranslation> = serde_json::from_value(json!([{"key":"birds","locale":"de","value":"%lld Vogelarten","path":[{"device":"iphone"}]}])).unwrap();
    ...
    assert_eq!(
        ...
        json!({"future":8,"variations":{"device":{"iphone":{"stringUnit":{"state":"translated","value":"%lld Vogelarten","futureUnit":3}},"other":{"stringUnit":{"state":"translated","value":"%lld Tiere"}}}}})
    );
}
...
fn duplicate_destinations_reject_every_overlapping_request_before_mutation() {
    ...
    let translations: Vec<xcstrings_mcp::model::translation::CompletedTranslation> = serde_json::from_value(json!([{"key":"birds","locale":"de","value":"A","path":[{"device":"iphone"}]},{"key":"birds","locale":"de","value":"B","path":[{"device":"iphone"}]}])).unwrap();
...
fn selector_conflict_and_invalid_path_have_specific_codes() {
    ...
    let translations: Vec<xcstrings_mcp::model::translation::CompletedTranslation> = serde_json::from_value(json!([{"key":"birds","locale":"de","value":"X","path":[],"plural_forms":{"other":"X"}},{"key":"birds","locale":"de","value":"X","path":[{"device":"toaster"}]}])).unwrap();
...
fn partial_plural_update_is_valid_and_explicit_blank_is_intentional() {
    let file = parser::parse(CATALOG).unwrap();
    let translations: Vec<xcstrings_mcp::model::translation::CompletedTranslation> = serde_json::from_value(json!([{"key":"birds","locale":"fr","value":"","plural_forms":{"other":"%lld oiseaux"}},{"key":"birds","locale":"de","value":"","path":[{"device":"iphone"}]}])).unwrap();
...
#[test]
fn absent_substitution_target_keeps_parent_references_new_and_only_submitted_leaf_ready() {
    ...
    let translations: Vec<xcstrings_mcp::model::translation::CompletedTranslation> = serde_json::from_value(json!([{"key":"count","locale":"de","value":"%arg Dinge","path":[{"substitution":"N"},{"plural":"other"}]}])).unwrap();
    ...
    assert_eq!(
        ...
        json!({"state":"translated","value":"%arg Dinge"})
    );
}
...
fn root_and_device_requests_overlap_before_either_is_applied() {
    let mut file = parser::parse(CATALOG).unwrap();
    let before = serde_json::to_string(&file).unwrap();
    let requests: Vec<xcstrings_mcp::model::translation::CompletedTranslation> = serde_json::from_value(json!([{"key":"birds","locale":"ja","value":"Birds","path":[]},{"key":"birds","locale":"ja","value":"Phone birds","path":[{"device":"iphone"}]}])).unwrap();
