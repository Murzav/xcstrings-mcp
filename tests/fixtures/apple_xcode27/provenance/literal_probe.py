...
R=Path(__file__).parent/'literal-delimiters';R.mkdir(exist_ok=True);C=Path('/Users/murzav/projects/rust/xcstrings-mcp-pr26/tests/fixtures/apple_xcode27');N='urn:oasis:names:tc:xliff:document:1.2';E.register_namespace('',N)
original=json.loads((C/'positive/catalog-matrix/source.xcstrings').read_text())
base={'sourceLanguage':'en','version':'1.0','strings':{key:{'extractionState':'manual','localizations':{'en':{'stringUnit':{'state':'translated','value':'EN '+key}},'fr':{'stringUnit':{'state':'translated','value':'FR '+key}}}} for key in ['plain|==|word','plain|==|plural.one','plain|==|device.iphone','plain|==|substitutions.COUNT.plural.one','a|==|b|==|plural.one','plain|==|unknown.x']}}
def run(d,stage,cmd):
